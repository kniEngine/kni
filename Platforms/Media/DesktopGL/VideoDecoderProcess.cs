        ...
        private void ParseTrackEntry(PipeBinaryReader reader, long entrySize)
        {
            ...
            if (trackNumber != -1)
            {
                switch (trackType)
                {
                    ...
                    default:
                        _tracks[trackNumber] = new MKVTrack(trackType);
                        break;
                }
            }
        ...
        private void ParseSimpleBlock(PipeBinaryReader reader, long blockSize, int clusterTimecode)
        {
            ...
            switch (track.Type)
            {
                case MKVTrackType.Video:
                    {
                        ReadVideoFrame(_videoFrameData, frameDataSize);
                    }
                    break;
                case MKVTrackType.Audio:
                    {
            ...
            Debug.WriteLine($"  SimpleBlock -> Track: {trackNumber}, TrackType: {track.Type}, Time: {absoluteTimecode}, Keyframe: {(flags & SimpleBlockFlags.Keyframe) != 0}, Size: {frameDataSize}");
        }

        public void ReadVideoFrame(byte[] frameData, long videoFrameDataSize)
        {
